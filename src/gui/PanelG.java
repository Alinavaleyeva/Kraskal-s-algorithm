...
public class PanelG extends JPanel {
    private java.util.List<Node1> nodes;
        ...
        addMouseListener(new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent e) {
                if (SwingUtilities.isLeftMouseButton(e)) {
                    Node1 node = findNode(e.getX(), e.getY());
                    if (node == null) {
                        nodes.add(new Node1(e.getX(), e.getY()));
                    } else {
                        selectedNode = node;
                    }
                    repaint();
                    ...
                    if (node != null) {
                        ...
                        Edge1 edge = findEdge(e.getX(), e.getY());
                        if (edge != null) {
                            removeEdge(edge);
                        }
                    }
                ...
                if (SwingUtilities.isLeftMouseButton(e)) {
                    if (selectedNode != null) {
                        ...
                        if (node != null && node != selectedNode) {
                            ...
                            edges.add(new Edge1(selectedNode, node, edgeWeight));
                        }
                        selectedNode = null;
                    }
                    repaint();
    ...
    private Node1 findNode(int x, int y) {
    ...
    private Edge1 findEdge(int x, int y) {
    ...
    private void removeNode(Node1 node) {
        nodes.remove(node);
        Iterator<Edge1> iterator = edges.iterator();
        while (iterator.hasNext()) {
            Edge1 edge = iterator.next();
    ...
    private void removeEdge(Edge1 edge) {
        edges.remove(edge);
    }
